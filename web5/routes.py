...
# 5.5, 增加一个路由 /admin/users
# 只有 id 为 0 的用户可以访问这个页面, 其他用户访问会定向到 /login
...
def route_users(request):
    ...
    if u.role == 1:
        ...
        user_html = ''.join(['<h3>id:{} | username: {} | password: {} </h3>'.format(u.id, u.username, u.password)for u in User.all()])
        body = body.replace('{{result}}', user_html)
        response = header + '\r\n' + body
        return response.encode(encoding='utf-8')
    else:
        return redirect('/login')

# 5.6, 在 /admin/users 页面中新增一个表单
...
def route_admin_update(request):
    ...
    if request.method == 'POST':
        form = request.form()
        log('routes-admin-form', form)
        userid = int(form.get('id', -1))
