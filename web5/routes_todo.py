...
def index(request):
    ...
    for t in todo_list:
        ...
        # 5.4, 在todo 界面上显示创建和修改事件, 格式如下(24小时制)
        # 11/02 10:31
        create_time = time.strftime('%m/%d %H:%M:%S', time.localtime(t.created_time))
        update_time = time.strftime('%m/%d %H:%M:%S', time.localtime(t.updated_time))

        s = '<h3>{} : {} | start:{} | update:{} | {} |{}</h3>'.format(t.id, t.title, create_time, update_time, edit_link, delete_link)
        todos.append(s)
